...
#' @export
#' @examples
#' data(exampleestimatedPowerer)
#' plotEstPwr(exampleestimatedPowerer)
#'
# Author: Xu Qiao
...
setMethod(f="plotEstPwr",
          signature="PEObject",
          definition=function(PEObject)
          {
            if(PEObject@resultType == "predictedPower")
              stop("Use 'linePredPwr/heatMapPredPWr' instead for estimated power.")
            estimatedPower <- PEObject@estPwr
            if(is.null(nrow(estimatedPower)))
              stop("Estimated power results not found in PEObject!")
            #determine dataType
            ...
            SummaryTable <- data.frame(
              ...
              A.P=apply(estimatedPower, 2, function(x) round(mean(x, na.rm=TRUE), 2)),
            ...
            power.boxplot <-
              ...
              ggtitle(label="Boxplot",
                      subtitle=paste("minLFC threshold:",
                                     attributes(estimatedPower)$minLFC)) +
