...
listEstPwr <- function(PEObject, decreasing = TRUE,  top = 20,  selected = NA){
  ...
  if(identical(selected, NA)) return(result[seq_len(top), ])
...
#'
listPredPwr <- function(PEObject, decreasing = TRUE, top = 20, selected = NA){
  result <- lapply(PEObject@predPwr, function(x) {
    temp <- data.frame(x, check.names = FALSE)
    temp <- temp[do.call(order, c(as.list(temp), decreasing = TRUE)),]
    
    if(identical(selected, NA)) return(temp[seq_len(top), ])
