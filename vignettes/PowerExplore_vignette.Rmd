...
```{r LinePlot}
plotPredictedPower(predictedPower, plotType = "lineplot", LFCscale = 0.5)
```

Heatmap (LFCscale = 0.5):
```{r Heatmap}
plotPredictedPower(predictedPower, plotType = "heatmap", LFCscale = 0.5)
```

It's also possible to observe the power tendencies in different LFC ranges if given minLFC and maxLFC.
